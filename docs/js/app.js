    ...
    function renderPortfolio(trades, quotes = {}) {
        ...
        sortedSymbols.forEach(sym => {
            ...
            row.innerHTML = `
                ...
                <td class="px-6 py-4 text-right ${getPriceColor(price, refPrice)}">
                    ${price > 0 ? formatNumber(price) : '--'}
                </td>
                <td class="px-6 py-4 text-right ${getPriceColor(price, refPrice)} text-xs">
                ...
                <td class="px-6 py-4 text-right ${pnl >= 0 ? 'text-red-500' : 'text-green-500'}">
