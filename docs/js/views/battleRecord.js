...
export const BattleRecord = {
        ...
        viewContainer.innerHTML = `
            <div class="flex flex-col space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="bg-white dark:bg-[#161b22] p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm transition-colors duration-300">
                        <p class="text-gray-500 text-xs mb-1">已結算總盈虧</p>
                        <h3 class="text-2xl font-mono font-bold" id="settled-total-pnl">--</h3>
                    </div>
                    <div class="bg-white dark:bg-[#161b22] p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm transition-colors duration-300">
                        <p class="text-gray-500 text-xs mb-1">勝率</p>
                        <h3 class="text-2xl font-mono font-bold text-blue-400" id="settled-win-rate">--</h3>
                    </div>
                    <div class="bg-white dark:bg-[#161b22] p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm transition-colors duration-300">
                        <p class="text-gray-500 text-xs mb-1">總獲利次數</p>
                        <h3 class="text-2xl font-mono font-bold text-red-400" id="settled-win-count">--</h3>
                    </div>
                    <div class="bg-white dark:bg-[#161b22] p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm transition-colors duration-300">
                        <p class="text-gray-500 text-xs mb-1">總虧損次數</p>
                        <h3 class="text-2xl font-mono font-bold text-green-400" id="settled-loss-count">--</h3>
                    </div>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            <thead class="bg-gray-50/50 dark:bg-gray-900/50 text-gray-400 dark:text-gray-500 text-xs uppercase">
                                <tr>
                                    <th class="px-6 py-4">代碼/名稱</th>
                                    <th class="px-6 py-4">結算日期</th>
                                    <th class="px-6 py-4 text-right">股數</th>
                                    <th class="px-6 py-4 text-right">賣出價</th>
                                    <th class="px-6 py-4 text-right">平均成本</th>
                                    <th class="px-6 py-4 text-right">盈虧</th>
                                    <th class="px-6 py-4 text-right">報酬率</th>
                                </tr>
                            </thead>
    ...
    async render() {
        ...
        if (settledRecords.length === 0) {
            settledBody.innerHTML = '<tr><td colspan="7" class="px-6 py-10 text-center text-gray-500">尚無結算資料</td></tr>';
            return;
        }
        ...
        settledRecords.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach(rec => {
            ...
            row.innerHTML = `
                ...
                <td class="px-6 py-4 text-right ${rec.pnl >= 0 ? 'text-red-500' : 'text-green-500'} font-bold">
                    ${rec.pnl >= 0 ? '+' : ''}${this.formatNumber(rec.pnl, 0)}
                </td>
                <td class="px-6 py-4 text-right ${rec.pnl >= 0 ? 'text-red-500' : 'text-green-500'} text-xs">
                    ${rec.roi.toFixed(2)}%
                </td>
            `;
